...
function build.fromHumanoid(humanoid: Humanoid)
	local character = humanoid.Parent

	local old = character:FindFirstChild("RagdollConstraints")
	if old then
		old:Destroy()
	end
    ...
    CollectionService:AddTag(humanoid, "HasRagdollConstraints")

    return ragdollConstraints
end

function build.removeFromHumanoid(humanoid: Humanoid): ()
    local constraintsFolder = humanoid.Parent:FindFirstChild("RagdollConstraints")

    if constraintsFolder and constraintsFolder:IsA("Folder") then
        constraintsFolder:Destroy()
    else
        warn("Constraints do not exist on this humanoid.")
    end

    CollectionService:RemoveTag(humanoid, "HasRagdollConstraints")
end
