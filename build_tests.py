...
class WatchHandler(PatternMatchingEventHandler):
	...
	def on_modified(self, event):
		...
		src_path = Path(original_src_path).relative_to("src")
		module_name = src_path.parent.name
		test_path = Path.joinpath(Path("test/src"), module_name + "_test", src_path)
...
def build_tests():
	rmtree("./test/src", ignore_errors=True)
	copytree("./src", "./test/src", dirs_exist_ok=True)
	for path in Path("./test/src").iterdir():
...
if __name__ == "__main__":
	...
	if has_watch:
		...
		observer = Observer()
		observer.schedule(WatchHandler(), "./src", recursive=True)
		observer.start()
